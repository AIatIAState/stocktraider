...
import { fetchAdminUpdateJobs, fetchAdminUpdateStatus, runAdminUpdate } from '../services/api'
...
export default function AdminPage(props: { disableCustomTheme?: boolean }) {
  ...
  const defaultStart = toLocalIsoDate(new Date(today.getTime() - 6 * MS_PER_DAY))
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to run update.'
      setError(message)
    } finally {
      setIsRunning(false)
    }
  ...
  const refreshJobs = useCallback(async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load job history.'
      setJobError(message)
    }
  }, [])
            ...
            <CardContent>
              <Stack spacing={1.5}>
                ...
                <Typography variant="body1" color="text.secondary">
                  Pull daily OHLCV data from yfinance and upsert it into the SQLite database.
                </Typography>
              </Stack>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <Stack spacing={2}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Select a date range, then confirm the warning before launching the job.
                </Typography>
                ...
                {error ? (
                  ...
                  <Alert severity="info">Update job is running. This may take several minutes.</Alert>
                ) : jobError ? (
                  <Alert severity="error">{jobError}</Alert>
                ) : summary ? (
                  <Alert severity="success">
                    Inserted {summary.rows_inserted} new rows for {summary.symbols} symbols.
                  </Alert>
                ) : null}
                ...
                <Box>
                  ...
                  >
                    {isRunning || jobStatus === 'running' ? 'Running update...' : 'Run update'}
                  </Button>
                </Box>
              </Stack>
            </CardContent>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <Alert severity="warning">
              This job writes to the production database and may take several minutes. Do not
              run it during active demos or heavy usage.
            </Alert>
            <FormControlLabel
              control={
                <Checkbox
