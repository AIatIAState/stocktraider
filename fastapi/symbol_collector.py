...
get_symbols.py
──────────────
Returns ~500-1000 training symbols and ~100 testing symbols for a given date.

Usage:
    from get_symbols import get_symbols
    train, test = get_symbols(date(2022, 1, 1))
"""
...
def get_symbols(target_date: date) -> tuple[list[str], list[str]]:
    ...
    train_symbols — 500-800 tickers: point-in-time S&P 500 + recent removals.
                    Includes companies that underperformed so the model learns
                    both good and bad patterns (avoids survivorship bias).

    test_symbols  — ~100 tickers: large-cap, sector-diverse, high-liquidity.
                    9 companies per GICS sector for a balanced simulation.
    """
    tables = _fetch_wiki_tables()
