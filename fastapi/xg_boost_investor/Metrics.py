...
def create_visualizations(df, year, output_dir='./', label=""):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}portfolio_value_{year}{label}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_dir}roi_over_time_{year}{label}.png', dpi=300, bbox_inches='tight')
    plt.close()


def export_metrics(metrics, year, output_dir='./', label=""):
    wb = Workbook()
    sheet = wb.active
    ...
    wb.save(f'{output_dir}portfolio_metrics_{year}{label}.xlsx')
